...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://formspdree.io/f/mknljydo", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          body: `New Contact Form Message from: ${formData.email} (${formData.name}) | PAGE: ${window.location.href} | CONTENT: ${formData.message}`,
        }),
      });
                ...
                <div className="bg-white shadow-lg p-6 sm:p-8 md:p-12">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button
                      ...
                      className="w-full bg-accent text-white py-4 px-8 hover:bg-primary transition-colors uppercase text-sm tracking-wider font-medium flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                    >
                      {isSubmitting ? (
                        <>
